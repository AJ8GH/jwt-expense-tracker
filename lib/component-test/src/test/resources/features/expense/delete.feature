Feature: Get all expenses
  ...
  Scenario: Happy path returns 204
    ...
    When a delete expense EXP1 request is sent with token AT1
    Then the response status is 204

  Scenario: Returns 204 when expense not found by ID
    When a delete expense EXP1 request is sent with token AT1
    Then the response status is 204
  ...
  Scenario: Returns 401 when token not authorised to access expense ID
    ...
    And the following create expense request "EXP1" is sent with token AT1
      ...
      | FOOD     | 100.00 | 2025-01-01 |
    And the response status is 201
    When an erroneous delete expense EXP1 request is sent with token AT2
